            ...
            <div class="card-body">
                <form action="{{ route('post.update',['id' => $post->id]) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <img width="100" height="140" src="{{ ($post->image == null) ? asset('images/image-not-found.jpg') : Storage::url($post->image) }}" id="image_preview" class="mt-2 img-fluid"
                             alt="">
                    </div>
                    <div class="form-group">
                                    ...
                                    <div class="col-md-4">
                                        <label>
                                            <input type="checkbox" name="post_type[]"
                                                   value="{{ $postType->id }}" {{ in_array($postType->id, $post->postTypes->pluck('id')->toArray()) ? 'checked' : '' }}>
                                            {{ $postType->name }}
                                        </label>
                                    </div>
                                @endforeach
{{--                                    {{ in_array($genre->id, $movie->genres->pluck('id')->toArray()) ? 'checked' : '' }}>--}}
                    ...
                    <div class="form-group">
                        <div class="row_content ">
                            <textarea name="content" id="content123" cols="30" rows="10" width="100%" height="1000px">{{ $post->content }}</textarea>
                        </div>
                    </div>
