                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        ...
                        <div class="dropdown tg-themedropdown tg-minicartdropdown">
                            ...
                            <div class="dropdown-menu tg-themedropdownmenu" aria-labelledby="tg-minicart">
                                <div class="tg-minicartbody">
                                    <form action="{{route('vnpay')}}" method="POST">
                                        ...
                                        <div class="tg-minicarproduct">
                                            <figure>
                                                <img width="100px" src="{{ Storage::url($item->options->get('image')) }}" alt="image description">

                                            </figure>
                                            <div class="tg-minicarproductdata">
                                                <h5><a href="javascript:void(0);">{{$item->name}}</a>
                                                <input type="hidden" name="name"  value="{{$item->name}}">
                                                </h5>
                                                <h6><a href="javascript:void(0);">{{$item->price}} vnd X {{$item->qty}}</a></h6>
                                ...
                                <div class="tg-minicartfoot">
                                    <a class="tg-btnemptycart" href="{{route('cart.delete')}}">
                                        <i class="fa fa-trash-o"></i>
                                        <span>Xóa sản phẩm của bạn</span>
                                    </a>
                                    <span class="tg-subtotal">Tổng tiền: <strong>{{floatval(Cart::total()) * 1000 }} </strong></span>
                                    <div class="tg-btns">
                                        <a class="tg-btn tg-active" href="{{route('cart')}}">Xem giỏ hàng</a>

                                        <button class="tg-btn" name="redirect" type="submit">Thanh toán</button>
                    ...
                    <div class="tg-searchbox">
                        <form class="tg-formtheme tg-formsearch">
                            <fieldset>
                                <input type="text" name="search" class="typeahead form-control" placeholder="Tìm sách theo tiêu đề,tác giả ...">
                                ...
                                <li class="menu-item-has-children">
                                    ...
                                    <ul class="sub-menu" id="categorySubMenu">
                                        ...
                                        <li class="menu-item-has-children" role="presentation">
                                            <a href="javascript:void(0);" class="category-link" data-slug="{{ $category->slug }}">{{ $category->name }}</a>
                                            @if ($category->children->isNotEmpty())
                                            <ul class="sub-menu">
                                                @foreach ($category->children as $child)
                                                <li>
                                                    <a href="#{{ $child->slug }}" aria-controls="{{ $child->slug }}" role="tab" data-toggle="tab">{{ $child->name }}</a>
